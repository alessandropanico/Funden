...
export class AtmService {
  ...
  //db test
  private apiUrl = 'https://testwallet.unikaexchange.com/api/wallet/atm/by-user';

  //db prod
  // private apiUrl = 'https://wallet.unikaexchange.com/api/wallet/atm/by-user';
  ...
  private fetchWithToken(): Observable<any> {
    ...
    return this.http.get<any>(this.apiUrl, { headers: this.createHeaders(token) }).pipe(
      tap(response => {
        if (response.ATM  === null) {
          console.log('📉 Nessun ATM trovato per questo utente.');
        } else {
